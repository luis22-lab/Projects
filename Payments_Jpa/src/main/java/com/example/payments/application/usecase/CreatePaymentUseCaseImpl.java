...
import com.example.payments.infraestructure.kafka.PaymentKafkaProducer;
...
public class CreatePaymentUseCaseImpl implements CreatePaymentUseCase {
    ...
    public Payment apply(Payment payment) {
        ...
        final var exists = paymentRepository.findOneByRequestId(payment.getRequestId());
        ...
        final var result = paymentRepository.save(payment).orElseThrow(() -> new DomainException(status_422,"Error to save payment"));
        paymentKafkaProducer.sendEventPayment(new PaymentCreatedEvent("PAYMENT-CREATED:"+result.getId(),result.getId().toString(),result.getAmount(),"test@domain.com", Instant.now()));
        return result;
